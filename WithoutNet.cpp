...
int begin(int idPrime, char *localNamePrime, long timestampOffset, bool verbose)
{
...
{
    Message message = Message(millis() + _timestampOffset, (MessageType)DATA, id, destId, payload, payloadLength);
...
void onIncomingMsgCharWritten(BLEDevice central, BLECharacteristic characterstic)
{
    ...
    if (messageChunkLength == 0)
    {
        ...
        {
            // TODO: Check if the timestamp exists in the queue
